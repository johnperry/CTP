...
import org.rsna.ctp.stdstages.anonymizer.dicom.DICOMWaveformProcessor;
...
/**
 * The DicomWaveformProcessor pipeline stage class. This class adds an image of the waveform
 * to a file that contains a waveform but no image.
 */
public class DicomWaveformProcessor extends AbstractPipelineStage implements Processor, Scriptable {
	 ...
	 */
	public DicomWaveformProcessor(Element element) {
		super(element);
		...
		if (fileObject instanceof DicomObject) {
			...
			if ((dicomScriptFile == null) || !dicomScriptFile.exists()
				...
				if (!dob.isImage() && dob.getDataset().contains(Tags.WaveformSeq)) {
					...
					AnonymizerStatus status = DICOMWaveformProcessor.process(file, file);
					if (status.isOK()) {
